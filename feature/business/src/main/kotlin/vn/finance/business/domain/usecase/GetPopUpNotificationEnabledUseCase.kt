...
class GetPopUpNotificationEnabledUseCase @Inject constructor(private val repository: PopUpNotificationRepository) :
    BaseUseCase<Unit, ResultModel<Boolean>>() {
    override fun execute(vararg params: Unit?): Flow<ResultModel<Boolean>> {
        return repository.getPopUpNotificationEnabled()
    }
}